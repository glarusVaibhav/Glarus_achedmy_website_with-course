...
export interface LiveCourseDetail {
  ...
  ctaText: string;
  classType: "Live Cohort" | "Full Bootcamp" | "Workshop";
  timeframeDays: number;
}

const SELF_PACED_COURSES: Course[] = [
  { id: "1", title: "Advanced LLM Architecture", category: "AI Engineering", difficulty: "Advanced", duration: "18h VOD", gradient: "from-purple-600/20 to-blue-600/20", linkUrl: "/course/ai-1", image: "/images/courses/llm-architecture.png", mode: "self-paced" },
  { id: "2", title: "Generative AI Application Engineering", category: "Web Development", difficulty: "Intermediate", duration: "24h VOD", gradient: "from-sky-500/20 to-indigo-500/20", linkUrl: "/course/Generative_AI_Application_Engineer", image: "/images/courses/generative-ai.png", mode: "self-paced" },
  { id: "3", title: "RAG & Vector Databases", category: "Data Science", difficulty: "Advanced", duration: "12h VOD", gradient: "from-emerald-500/20 to-teal-500/20", linkUrl: "/course/ai-3", image: "/images/courses/rag-vector-db.png", mode: "self-paced" },
  { id: "4", title: "Machine Learning Math Foundations", category: "AI Engineering", difficulty: "Beginner", duration: "32h VOD", gradient: "from-orange-500/20 to-rose-500/20", linkUrl: "/course/ai-2", image: "/images/courses/ml-math.png", mode: "self-paced" },
  { id: "5", title: "Smart Contract Security Testing", category: "Web3", difficulty: "Expert", duration: "15h VOD", gradient: "from-violet-500/20 to-fuchsia-500/20", linkUrl: "/course/ai-4", image: "/images/courses/smart-contracts.png", mode: "self-paced" }
];
...
export default function InteractiveCourses() {
  ...
  return (
    <section className="w-full py-8 md:py-12 bg-background overflow-hidden relative z-10 selection:bg-purple-500/30 text-text">
        ...
        >
          <div className="max-w-4xl">
            ...
            <p className="text-xl text-subtext font-medium leading-relaxed">
              Join expert-led live cohorts or learn at your own pace. Guaranteed to elevate your engineering status.
            </p>
          </div>
      ...
      {/* ───────────────────────────────────────────────────────────── */}
      {/* MODE 1: SELF-PACED CAROUSEL (UNTOUCHED)                        */}
      {/* ───────────────────────────────────────────────────────────── */}
      {activeMode === "self-paced" && (
        <div
          ref={containerRef}
          className="pl-6 md:pl-[max(1.5rem,calc((100vw-80rem)/2))] pb-4 overflow-x-auto md:overflow-hidden scrollbar-none select-none min-h-[380px]"
        >
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="flex gap-6 cursor-grab active:cursor-grabbing pb-4 w-max pr-12 md:pr-32"
            >
              {filteredSelfPaced.map((course, idx) => (
                <motion.div
                  ...
                  className="w-[300px] md:w-[400px] bg-card rounded-3xl border border-card/60 overflow-hidden shadow-xl flex flex-col group relative select-none cursor-pointer"
                >
                  <div
                    draggable={false}
                    className="h-48 md:h-56 w-full relative overflow-hidden flex items-center justify-center border-b border-card/60 select-none bg-background"
                  >
                    {course.image ? (
                      <Image
                        src={course.image}
                  ...
                  <div className="p-6 md:p-8 flex flex-col flex-1 relative bg-gradient-to-b from-card to-background select-none">
                    ...
                    <h3 className="text-xl md:text-2xl font-black text-text mb-4 leading-tight group-hover:text-primary transition-colors">
                      {course.title}
                    </h3>

                    <div className="flex items-center justify-between text-sm font-semibold text-subtext mt-auto mb-6">
                      <span className="flex items-center gap-1.5">
                        <Clock className="w-4 h-4 text-primary" /> {course.duration}
                      </span>
                    </div>

                    <div className="pt-5 border-t border-card/60 flex items-center justify-between">
                      ...
                      <div className="w-10 h-10 rounded-full bg-background border border-card flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors group-hover:border-primary">
            ...
            <div className="flex-1 w-full space-y-5">
              {filteredLive.length === 0 ? (
                <div className="w-full py-16 px-8 rounded-3xl border border-card/60 bg-card/40 backdrop-blur-md flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-xs text-subtext">Try clearing your active filters to see all available cohorts.</p>
                </div>
              ) : (
